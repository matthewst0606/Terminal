...
extension Terminal {
    ...
    // ------- clears one row in the output box -------
     func clearline() {
         if !output.isEmpty { output = output.dropLast() }
    }
    
    // ------- appends user input to output box -------
...
// used for displaying custom output views
extension Terminal {
...
extension Terminal {
    ...
    func outputError(
        ...
        output.append(.init(
            kind: .error(command: command, message: message),
            prompt: prompt
        ))
    }
}
